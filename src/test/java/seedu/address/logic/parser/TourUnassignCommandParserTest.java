...
public class TourUnassignCommandParserTest {

    private static final String MESSAGE_INVALID_FORMAT =
            String.format(MESSAGE_INVALID_COMMAND_FORMAT, TourUnassignCommand.MESSAGE_USAGE);

    private TourUnassignCommandParser parser = new TourUnassignCommandParser();

    @Test
    public void parse_validArgs_returnsUnassignTourCommand() {
        assertParseSuccess(parser, "1 tour/1",
                new TourUnassignCommand(INDEX_FIRST_CONTACT, INDEX_FIRST_TOUR));
    }

    @Test
    public void parse_validArgsWithWhitespace_returnsUnassignTourCommand() {
        assertParseSuccess(parser, "  1  tour/2  ",
                new TourUnassignCommand(INDEX_FIRST_CONTACT, INDEX_SECOND_TOUR));
    }
